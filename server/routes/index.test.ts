  ...
  it('renders the summary tiles when the summary service returns counts', async () => {
    ...
    return request(app)
      ...
      .expect(res => {
        ...
        expect(res.text).toContain('Containers stored on site')
        // counts over 999 take a comma separator
        expect(res.text).toContain('3,000')
        expect(res.text).not.toContain('>3000<')
        expect(res.text).toContain('Containers due to be transferred out')
        expect(res.text).toContain('Containers due to be disposed of')
